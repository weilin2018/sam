...
def get_vector_list_index_of_extreme_events(event_data):
    ...
    return np.array(extreme_event_index_matrix)
...
def remove_consecutive_days(event_data, event_data_idx):
    ...
    if len(event_data) != len(event_data_idx):
        print("ERROR! Event data and list of idx event data are not of the same length!")
        sys.exit(1)
...
def event_synchronization(event_data, tau_max=10, min_num_sync_events=10):
    num_time_series= len(event_data)
    adj_matrix=np.zeros((num_time_series,num_time_series),dtype=int)
    double_tau_max=2*tau_max
    tot_num_events=len(event_data)
        ...
        for j, ind_list_e2 in enumerate(extreme_event_index_matrix):
            ...
            for m, e1_ind in enumerate(ind_list_e1[1:-1], start=1):
                ...
                for n,e2_ind in enumerate(ind_list_e2[1:-1], start=1):
                    ...
                    tau=min(d_11_past, d_11_next, d_22_past, d_22_next, double_tau_max) /2
                    #print(tau, d_11_past, d_11_next, d_22_past, d_22_next, double_tau_max)
...
def event_synchronization_one_series(extreme_event_index_matrix, ind_list_e1, i, tau_max=10, min_num_sync_events=10):
    double_tau_max=2*tau_max
    ...
    for j, ind_list_e2 in enumerate(extreme_event_index_matrix):
        ...
        sync_events= event_sync(ind_list_e1, ind_list_e2, tau_max, double_tau_max)
...
def event_sync(ind_list_e1, ind_list_e2, tau_max, double_tau_max):
...
def prepare_es_input_data(event_data):
    """
...
def parallel_event_synchronization(event_data, tau_max=10, min_num_sync_events=1, job_id=0, num_jobs=1, savepath="./E_matrix.npy", null_model=None):
    num_time_series= len(event_data)
    ...
    parallelArray= (Parallel(n_jobs=num_cpus_avail,backend=backend)
                    (delayed(event_synchronization_one_series)
                      (extreme_event_index_matrix, e1, start_arr_idx + i, tau_max, min_num_sync_events)
...
def event_sync_reg(ind_list_e1, ind_list_e2, tau_max, double_tau_max):
...
def es_reg(es_r1, es_r2, taumax, ):
    ...
    comb_e12=np.array(list(product(es_r1, es_r2)))
    backend='multiprocessing'
    # backend='loky'
    #backend='threading'
    num_cpus_avail=mpi.cpu_count()
    ...
    t21=np.zeros(num_tp)
    t=np.zeros(num_tp)
    for (t_e, t12_e, t21_e) in parallelArray:
        t[t_e]+=1
        t12[t12_e]+=1
        t21[t21_e]+=1
    return t, t12, t21
...
def get_null_model_adj_matrix_from_E_matrix(E_matrix_ij, num_time_series,
                                   null_model_path='./gpcp_null_model_mnoe_404_threshold_001_tau_max10.npy',
                                   savepath='./adj_matrix/adj_matrix_null_model.npy'):
    adj_matrix=np.zeros((num_time_series,num_time_series),dtype=int)

    null_model=np.load(null_model_path)

    print('Null models shape: ', null_model.shape)
    for sync_events_ij in tqdm(E_matrix_ij):
        num_sync_events_ij, num_events_i, num_events_j, i, j = sync_events_ij
    ...
    print(f'Finished computing Adjency Matrix for Null model with {num_time_series} vector of time series!')
...
def null_model_one_series(i, min_num_events,l, num_permutations, tau_max, double_tau_max):
    list_thresholds_i=[]
    for j in range(min_num_events,i + 1):
        ...
        for k in range(num_permutations):
            ...
            cor[k] = event_sync(ind_list_e1, ind_list_e2, tau_max, double_tau_max)
...
def null_model_distribution(length_time_series, tau_max=10, 
    ...
    parallelArray= (Parallel(n_jobs=num_cpus_avail,backend=backend)
                    (delayed(null_model_one_series)
                      (i, min_num_events,l, num_permutations, tau_max, double_tau_max)
    ...
    np.save(savepath + f'_threshold_05.npy', P1)
    np.save(savepath + f'_threshold_02.npy', P2)
    np.save(savepath + f'_threshold_01.npy', P3)
    np.save(savepath + f'_threshold_005.npy', P4)
    np.save(savepath + f'_threshold_001.npy', P5)

    return P1, P2, P3, P4, P5


def null_model_cdf_one_series(i, min_num_events,l, num_permutations, tau_max, double_tau_max):
    list_thresholds_i=[]
    for j in range(min_num_events,i + 1):
        season1 = np.zeros(l, dtype = "bool")
        season2 = np.zeros(l, dtype = "bool")
...
#%% Past processing
def  construct_full_matrix_of_parts(num_jobs,filename, savepath=None):
