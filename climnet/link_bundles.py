...
def spherical_kde(link_points, coord_rad, bw_opt,rtol=1e-2):
    """
    ...
    kde = KernelDensity(metric='haversine', kernel='gaussian', 
                        algorithm='ball_tree',bandwidth=bw_opt, rtol=rtol)
...
def link_bundle_null_model_link_number(coord_rad, num_links,
    ...
    if os.path.exists(savepath):
        return None
    bw_opt = scott_factor * num_links**(-1./(2+4)) # Scott's rule of thumb
    ...
    for s in range(num_rand_permutations):
        ...
        null_model_bundles[s,:] = spherical_kde(all_links_rand, coord_rad, bw_opt)

    stats = compute_stats(runs=null_model_bundles)

    # print(std.shape, perc999.shape)


    np.save(savepath, stats)
...
def link_bundle_null_model(adj_matrix, coord_rad,
                           link_bundle_folder, filename, num_rand_permutations=1000,
    ...
    (Parallel(n_jobs=num_cpus,backend=backend)
                (delayed(link_bundle_null_model_link_number)
                  (coord_rad, num_links,
                  link_bundle_folder, filename, num_rand_permutations )
...
def link_bundle_one_location(adj_matrix, idx_node, coord_rad,
                             folder, filename, scott_factor=0.2,
                             perc=999, plot=False):
    """
    Args:
    -----
    ...
    # KDE
    Z_node = spherical_kde(link_coord, coord_rad, bw_opt)
    ...
    results = (
        ...
        (delayed( link_bundle_one_location)
            (adj_matrix, idx_node, coord_rad,
             null_model_folder, null_model_filename, scott_factor, perc)
