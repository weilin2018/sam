...
class Dendro_tree():
    def __init__(self, Z, node_levels):
    ...
    def get_node_level_Z_dict(self, node_levels ):
        ...
        full_node_levels=np.concatenate(([ground_level], node_levels), axis=0)
        ...
        for leaf_id in leaf_nodes:
            ...
            for node in foreward_path[:]:
                ...
                if Z_id not in dict_node_Z:
                    dict_node_Z[Z_id]={'level': nl, 'group_id':g_id_level}
    ...
    def get_intersect_node_ids(self, sel_node_ids):
        ...
        result_dict={'level': level_arr, 'is_node':is_node_ids, 'loc_id': node_ids}
        return result_dict
    ...
    def get_split_groups(self, node_ids_arr, abs_th=2, rel_th=0.1):
        ...
        for node_ids in node_ids_arr:
            ...
            u_Z_ids= np.intersect1d(Z_ids, u_Z_ids) # sorted unique array of Z_ids
    ...
    def get_split_groups_bak(self, node_ids_arr, abs_th=10, rel_th=1):
        ...
        for node_ids in node_ids_arr:
            ...
            u_Z_ids= np.intersect1d(Z_ids_1, u_Z_ids) # sorted unique array of Z_ids
    ...
    def plot_hist_is_nodes(self, sel_node_ids, th=2, ax=None, title='', savepath=None):
        ...
        Z_ids, frequencies,  Z_most_occ=self.get_hist_sel_nodes(sel_node_ids, th=th)
        names=[]
        for Z_id in Z_ids:
            level=self.dict_node_Z[Z_id]['level']
...
class Dendrogram_ES (ES_Graph_tool):
    ...
    def create_Z_linkage(self,group_levels):
        ...
        """
        num_levels = len(group_levels)
        num_nodes= len(group_levels[0])
        X = [[i] for i in group_levels[0]]
        ...
        for li, level in enumerate(group_levels):
            ...
            group_idx=li+1
            if li==0:
                Z,ind_cluster_z, num_nodes_z, z_index=self.linkage_first_level(level, group_idx)
            else:
                this_level_Z,ind_cluster_z, num_nodes_z, z_index=self.Z_on_upper_level(level,
                                                                    ind_cluster_z,
                                                                    num_nodes_z,
                                                                    group_idx,
                                                                    z_index)
                    
                if len(this_level_Z)>0:
                    if len(Z)<1:
                        Z=this_level_Z
                    else:
                        Z=np.concatenate((Z, this_level_Z),axis=0)
        ...
        for group_number, node_ids in clustered_nodes.items():
            if len(node_ids)<2:
                ...
                for ci, node_id in enumerate(node_ids[2:]):
                    ...
                    Z.append(this_link)
                    z_index+=1
                ind_cluster_z[cg]=z_index
                num_nodes_cluster_z[cg]=len(node_ids)
            cg+=1
        if len(Z)>1:
            Z=np.vstack(Z)
        return Z, ind_cluster_z, num_nodes_cluster_z, z_index
    ...
    def Z_on_upper_level(self, group_levels, ind_translate,num_nodes, group_idx, z_index ):
        ...
        for group_number, node_ids in clustered_nodes.items():
            if len(node_ids)<2:
                ...
                ind_cluster_z[cg]=id_1
                num_nodes_cluster_z[cg]= num_nodes[node_ids[0]]     #len(node_ids)
            else:
                id_1=ind_translate[node_ids[0]]
    ...
    def sel_node_levels(self, sel_node_ids):
        ...
        for i, leaf_id in enumerate(sel_node_ids):
            foreward_path=self.foreward_node_levels(self.node_levels, leaf_id)
    ...
    def sel_group_levels(self, sel_node_ids):
        ...
        sel_group_levels=self.get_hierarchical_data_from_nodes(red_node_levels)
    ...
    def get_link_colors(self, Z, group_levels, cut_level, color_branch=True):
        ...
        * if the colors of the connected clusters match, use that color for link
        ...
        ground_level_ids=group_levels[0] # Attention check again if this is true!

        cmap,norm = self.discrete_cmap(0, num_clusters)
        colors=np.array([rgb2hex(cmap(norm(rgb))) for rgb in range(num_clusters)])
    ...
    def get_id_to_coord(self, Z, ddata, ax=None):
        ...
        id_to_coord = dict(zip(ddata['leaves'], [leave_coords[idx] for idx in order])) # <- main data structure
    ...
    def plot_path(self, ddata, node_path, ax, color, label=None):
        ...
        for i, node_id in enumerate(node_path):
            ...
            if i<len(node_path)-1:
                ...
                xcoord = [this_node_coord[0], this_node_coord[0], next_node_coord[0]]
                ycoord = [this_node_coord[1], next_node_coord[1], next_node_coord[1]]
                ax.plot(xcoord, ycoord, lw=lw, ls='dashed', color=color, label=None)
        if xcoord is not None:
            ax.plot(xcoord, ycoord, lw=lw, ls='dashed', color=color, label=label)
    ...
    def plot_dendrogram(self, Z=None, group_levels=None, cut_level=None, title=None,
        ...
        if cut_level is not None and plot_Z:
            link_colors, cluster_colors, num_clusters=self.get_link_colors(Z, group_levels, 
                                                            cut_level, 
        ...
        if node_ids is not None:
            ...
            id_to_coord=self.get_id_to_coord(Z, ddata, ax=None)  # if ax is not None, id numbers are plotted
        ...
        y_title=1.05
        if title is None:
            ax.set_title(f'Hierarchical Clustering Dendrogram with {num_clusters} groups', y=y_title)
        else:
...
if __name__ == "__main__":
    ...
    # %%
    print('Loading Data')
    ds = Dendrogram_ES(group_levels=group_levels)
    # %%
    sel_node_ids=np.arange(0,7000, 500)
    Z_ids=ds.d_tree.get_all_Z_ids_leaf_ids(sel_node_ids)
    # ds.d_tree.get_Z_ids_of_level(5)
    # %%
    sel_group_levels=ds.sel_group_levels(sel_node_ids)
    sel_Z=ds.create_Z_linkage(sel_group_levels)
    ddata=ds.plot_dendrogram(Z=sel_Z, group_levels=sel_group_levels, cut_level=2)

    # i_dict=ds.d_tree.get_intersect_node_ids(sel_Z)
    # ds.d_tree.plot_hist_is_nodes(sel_node_ids)


    # %%
    ddata=ds.plot_dendrogram()